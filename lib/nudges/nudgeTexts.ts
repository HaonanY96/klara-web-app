...
export function getRandomVariant(type: NudgeType): { message: string; subMessage: string } {
  ...
  const message = messages[Math.floor(Math.random() * messages.length)] || NUDGE_TEXTS[type].message;
  const subMessage = subMessages[Math.floor(Math.random() * subMessages.length)] || NUDGE_TEXTS[type].subMessage;
